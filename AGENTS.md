...
Comment the *why*, not the *what* — aim for roughly one inline comment per 40 lines of JS, and skip JSDoc annotations (no build step or type checker here, so they only rot). Any constraint documented as "always do X" must also appear as a comment at the code site it protects, since the docs are not open when someone edits the file. Do not add comments to `Certificate.html` (generated by `build_study_app.py`) or the `index.backup*.html` snapshots. Note that `--` is illegal inside `<!-- -->` and `/* */` cannot nest — a malformed comment breaks these single-file pages silently, so reload the page after editing them.
